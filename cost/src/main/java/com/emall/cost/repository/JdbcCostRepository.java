...
public class JdbcCostRepository implements CostRepository {
    ...
    public CostBudget saveBudget(CostBudget budget) {
        jdbcTemplate.update("""
                ...
                budget.currency(), budget.alertThresholdPercent(), budget.active(),
                Timestamp.from(budget.createdAt()), Timestamp.from(budget.updatedAt()));
        return budget;
    }
    ...
    public Optional<CostOptimizationAction> findAction(long actionId) {
        return jdbcTemplate.query("SELECT * FROM cost_optimization_action WHERE action_id = ?",
    ...
    private CostBudget mapBudget(ResultSet rs, int rowNum) throws SQLException {
        return new CostBudget(
                ...
                rs.getInt("alert_threshold_percent"),
    ...
    private CostOptimizationAction mapAction(ResultSet rs, int rowNum) throws SQLException {
        return new CostOptimizationAction(
                rs.getLong("action_id"),
                rs.getString("service_name"),
                CostSignalType.valueOf(rs.getString("signal_type")),
                CostActionType.valueOf(rs.getString("action_type")),
                CostActionStatus.valueOf(rs.getString("status")),
                rs.getInt("priority"),
