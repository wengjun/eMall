...
1. `POST /api/orders` 使用 `requestId` 幂等创建订单，并通过 `clientType=WEB|APP` 区分 Web 和手机 App 下单。
2. 订单服务读取价格和优惠快照。
3. 订单服务调用库存服务预占库存。
4. 预占成功后订单进入 `CREATED`。
...
手机 App 下单时将 `clientType` 改为 `APP`。老客户端不传 `clientType` 时，订单服务默认按 `WEB` 处理。
...
HTTP/Dubbo + Nacos。`gateway` 会追加安全响应头作为后端兜底。
