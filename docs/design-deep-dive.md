...
本文用于学习、面试讲解和开源展示，重点说明 eMall 的设计理由、核心数据流、关键实现细节，以及和常见电商
项目方案相比的优势与不足。当前服务边界和运行拓扑以[架构设计](architecture.md)为唯一事实源。
...
总体架构图、分层和完整服务边界见[架构设计](architecture.md#总体架构图)。本文不复制该图和模块清单，
下面直接下钻到交易、支付、搜索同步、库存、事件、补偿和可观测数据流。
...
- `identity` 是账户状态的唯一权威服务，注册请求通过 `Idempotency-Key` 防止重复创建。
- 待注册账户、凭据、生命周期记录和 Outbox 在同一个本地事务提交，用户档案 ACK 前账户不能登录。
- `user` 按账户 ID 选择分片后，在一个事务中写消费幂等记录、版本水位、档案和 ACK Outbox。
- 停用、恢复、关闭和删除都由身份事件驱动；删除先擦除用户档案，再删除身份凭据和主体信息。
- 分布式租约按 256 个逻辑分区持续对账，未收敛状态会被重发并触发 Prometheus 告警。
