...
本文是当前系统边界和运行拓扑的架构事实源。设计理由、具体数据流、竞品比较和面试表达统一放在
[设计深度说明](design-deep-dive.md)，不在两份文档中重复维护同一结构。
...
| 数据层 | MySQL、Redis、Elasticsearch、Kafka、对象存储 | 分库分表、缓存、搜索、事件流、冷热分层 |
...
| 服务 | 职责 | 高并发关注点 |
| --- | --- | --- |
| `identity` | 账户注册、凭据、会话和账户生命周期权威状态 | 注册幂等、令牌撤销、跨服务生命周期收敛 |
| `user` | 用户档案投影、昵称和敏感信息保护 | 事件幂等、分片事务、敏感字段加密 |
...
真实生产中，这套逻辑应结合 Sentinel 指标、实例健康检查、自动扩缩容事件和发布系统一起使用。
