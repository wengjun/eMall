...
- 本题要围绕「如何处理数据合规删除和历史订单保留？」展开，不要只复述分类模板。
...
- 这张图用于把「如何处理数据合规删除和历史订单保留？」放回生产链路中理解，重点看入口、状态、数据和恢复闭环。
...
- 回答「如何处理数据合规删除和历史订单保留？」时，要从这些模块里选一个主链路做例子。
...
- 回答「如何处理数据合规删除和历史订单保留？」时，不能只给单一方案，要先说明约束、目标和失败边界。
...
- 针对「如何处理数据合规删除和历史订单保留？」，先确认它影响的是正确性、稳定性、性能、安全还是成本。
...
- 回答「如何处理数据合规删除和历史订单保留？」时，要主动放到 10 亿用户、1 亿 DAU、100W 峰值并发的背景下思考。
