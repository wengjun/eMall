...
高级追问要讲清取舍条件、反例、成本、风险和验证办法。
围绕这道题，要进一步把概念、项目实现、线上风险和验证闭环连起来。

### 深度和广度补充

- 先承认没有银弹，方案必须绑定业务目标和约束。
- 再列备选方案，比较一致性、可用性、复杂度、成本和团队能力。
- 随后说明你会如何小步验证，而不是一开始就大规模重构。
- 最后给出失败预案和度量指标，让决策可回滚、可审计。

### 图片讲解

![二次补强图解](../../assets/governance-platform.svg)

- 图中体现治理平台同时管理策略、流程、执行和观测。
- 读图时要说明架构决策不是一次性选择，而是持续治理。
- 高分回答要能说出方案在什么条件下不成立。
...
public record ArchitectureOption(
    ...
    int score() {
        return reliability * 3 - complexity * 2 - cost;
    }
}

final class ArchitectureDecision {

    ArchitectureOption choose(ArchitectureOption left, ArchitectureOption right) {
...
- 本题要围绕「你为什么用 OpenSearch，而不是 MySQL like 查询？」展开，不要只复述分类模板。
...
- 这张图用于把「你为什么用 OpenSearch，而不是 MySQL like 查询？」放回生产链路中理解，重点看入口、状态、数据和恢复闭环。
...
- 回答「你为什么用 OpenSearch，而不是 MySQL like 查询？」时，要从这些模块里选一个主链路做例子。
...
- 回答「你为什么用 OpenSearch，而不是 MySQL like 查询？」时，不能只给单一方案，要先说明约束、目标和失败边界。
...
- 针对「你为什么用 OpenSearch，而不是 MySQL like 查询？」，先确认它影响的是正确性、稳定性、性能、安全还是成本。
...
- 回答「你为什么用 OpenSearch，而不是 MySQL like 查询？」时，要主动放到 10 亿用户、1 亿 DAU、100W 峰值并发的背景下思考。
