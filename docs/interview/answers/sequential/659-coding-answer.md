...
- 本题要围绕「手写一个可关闭的后台 worker。」展开，不要只复述分类模板。
...
- 这张图用于把「手写一个可关闭的后台 worker。」放回生产链路中理解，重点看入口、状态、数据和恢复闭环。
- 面试时可以先按图说明主路径，再补失败路径、监控指标和回滚手段。

### 贴合本题的实现示例

```java
final class ClosableWorker implements AutoCloseable {
    private volatile boolean running = true;
    private final Thread thread = Thread.ofPlatform().start(this::loop);
...
- 回答「手写一个可关闭的后台 worker。」时，要从这些模块里选一个主链路做例子。
...
- 回答「手写一个可关闭的后台 worker。」时，不能只给单一方案，要先说明约束、目标和失败边界。
...
- 针对「手写一个可关闭的后台 worker。」，先确认它影响的是正确性、稳定性、性能、安全还是成本。
...
- 回答「手写一个可关闭的后台 worker。」时，要主动放到 10 亿用户、1 亿 DAU、100W 峰值并发的背景下思考。
