...
围绕「CountDownLatch、CyclicBarrier、Semaphore 分别适合什么场景？」，高分答案不能停在概念定义，而要把「线程安全、锁、CAS、线程池、隔离、超时和并发容量」讲成一条可验证的工程链路。
...
本题复习重点：`CountDownLatch`、`CyclicBarrier`、`Semaphore` 分别适合什么场景？
