...
class IdempotencyContractIT {
    ...
    void shouldKeepOrderPaymentAndCallbackIdempotentAgainstRunningGateway() throws Exception {
        ...
        long userId = ProductionHttpGate.postJson(baseUrl, "/api/users", Map.of(
                "mobile", "177" + suffix.substring(suffix.length() - 8),
                "nickname", "idempotency"), null)
                .path("data")
                .path("userId")
                .asLong();
        ProductionHttpGate.postJson(baseUrl, "/api/prices", Map.of(
                "skuId", skuId,
                "listPrice", BigDecimal.valueOf(2999, 0),
                "salePrice", BigDecimal.valueOf(2599, 0),
                "currency", "CNY",
                "active", true), null);
        ...
        Map<String, Object> paymentRequest = Map.of(
                "requestId", "idem-payment-" + suffix,
                "orderId", firstOrder.path("data").path("orderId").asLong(),
                "userId", userId,
                "amount", firstOrder.path("data").path("payableAmount").decimalValue(),
                "channel", "mock");
        ...
        Map<String, Object> callbackRequest = Map.of(
                "channelTradeNo", "idem-trade-" + suffix,
                "paidAmount", firstOrder.path("data").path("payableAmount").decimalValue());
        JsonNode firstCallback = ProductionHttpGate.postJson(baseUrl,
                "/api/payments/" + paymentId + "/callbacks", callbackRequest, null);
        JsonNode secondCallback = ProductionHttpGate.postJson(baseUrl,
                "/api/payments/" + paymentId + "/callbacks", callbackRequest, null);
        assertThat(secondCallback.path("data").path("status").asText())
                .isEqualTo(firstCallback.path("data").path("status").asText())
                .isEqualTo("SUCCEEDED");
    }
}
