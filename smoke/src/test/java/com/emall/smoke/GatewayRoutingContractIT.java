...
class GatewayRoutingContractIT {
    ...
    void shouldRouteCoreCommerceApisThroughGateway() throws Exception {
        ...
        long skuId = ProductionHttpGate.postJson(baseUrl, "/api/products",
                ...
                null).path("data").path("skuId").asLong();
        ...
        assertThat(ProductionHttpGate.getJson(baseUrl, "/api/products/" + skuId).path("data").path("skuId").asLong())
                .isEqualTo(skuId);
        assertThat(
                ProductionHttpGate.postJson(baseUrl, "/api/prices/quotes", Map.of("skuId", skuId, "quantity", 2), null)
                        .path("data").path("subtotal").decimalValue())
                .isEqualByComparingTo(BigDecimal.valueOf(3598, 0));
        assertThat(ProductionHttpGate
                .postJson(baseUrl, "/api/marketing/quotes",
                        Map.of("userId", userId, "orderAmount", BigDecimal.valueOf(3598, 0)), null)
                .path("data").path("payableAmount").decimalValue()).isPositive();
    }
}
