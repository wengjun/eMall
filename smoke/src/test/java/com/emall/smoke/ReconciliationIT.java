...
class ReconciliationIT {
    ...
    void shouldIngestAndReconcileChannelStatementAgainstRunningPaymentService() throws Exception {
        ...
        Map<String, Object> statement = Map.of("channel", envOrDefault("EMALL_RECONCILIATION_CHANNEL", "mock"),
                "channelTradeNo", tradeNo, "paymentId", paymentId, "amount", new BigDecimal(amount), "statementType",
                envOrDefault("EMALL_RECONCILIATION_STATEMENT_TYPE", "PAYMENT"), "occurredAt", Instant.now());

        ProductionHttpGate.postJson(paymentUrl(), "/internal/operations/payments/channel-statements", statement, token);
        ProductionHttpGate.post(paymentUrl(), "/internal/operations/payments/reconcile-channel-statements?limit=10",
                token);
    }
