...
class InternalOperationsSecurityIT {
    ...
    void shouldRejectInvalidInternalOperationTokenAgainstRunningServices() throws Exception {
        ...
        assertThat(ProductionHttpGate.postStatus(orderUrl, "/internal/operations/orders/retry-pending?limit=1",
                "invalid-token")).isEqualTo(403);
        assertThat(ProductionHttpGate.postStatus(inventoryUrl,
                "/internal/operations/inventory/release-expired-reservations?limit=1", "invalid-token")).isEqualTo(403);
        assertThat(ProductionHttpGate.postStatus(paymentUrl,
                "/internal/operations/payments/retry-order-confirmation?limit=1", "invalid-token")).isEqualTo(403);
    }
}
