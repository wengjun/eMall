...
public final class CheckoutSmokeApplication {
    ...
    public static void main(String[] args) throws Exception {
        String baseUrl = args.length > 0 ? args[0] : System.getenv().getOrDefault("EMALL_BASE_URL",
                "http://localhost:8080");
    ...
    private void run() throws IOException, InterruptedException {
        ...
        JsonNode user = post("/api/users", Map.of(
                "mobile", mobile,
                "nickname", "smoke"));

        post("/api/prices", Map.of(
                "skuId", 10001L,
                "listPrice", BigDecimal.valueOf(3999, 0),
                "salePrice", BigDecimal.valueOf(3799, 0),
                "currency", "CNY",
                "active", true));

        post("/api/inventory/10001/stock", Map.of("quantity", 10));

        JsonNode order = post("/api/orders", Map.of(
                "requestId", "smoke-order-" + suffix,
                "userId", user.path("data").path("userId").asLong(),
                "skuId", 10001L,
                "quantity", 1));
        BigDecimal payableAmount = order.path("data").path("payableAmount").decimalValue();

        JsonNode payment = post("/api/payments", Map.of(
                "requestId", "smoke-payment-" + suffix,
                "orderId", order.path("data").path("orderId").asLong(),
                "userId", user.path("data").path("userId").asLong(),
        ...
        System.out.printf("Smoke checkout succeeded. orderId=%d, paymentId=%d, fulfillmentId=%d%n",
                order.path("data").path("orderId").asLong(),
                payment.path("data").path("paymentId").asLong(),
