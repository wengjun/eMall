...
class JdbcPlatformOpsRepository implements PlatformOpsRepository {
    ...
    public BackupPlan saveBackupPlan(BackupPlan plan) {
        jdbcTemplate.update("""
                ...
                """, plan.planId(), plan.databaseName(), plan.backupType(), plan.retentionDays(),
                plan.status().name(), Timestamp.from(plan.createdAt()), Timestamp.from(plan.updatedAt()));
        return plan;
    }

    @Override
    public Optional<BackupPlan> findBackupPlan(long planId) {
        return jdbcTemplate.query("SELECT * FROM backup_plan WHERE plan_id = ?", this::mapBackupPlan, planId)
                .stream().findFirst();
    }
    ...
    public Optional<DatabaseOperation> findDatabaseOperation(long operationId) {
        return jdbcTemplate.query("SELECT * FROM database_operation WHERE operation_id = ?",
                this::mapDatabaseOperation, operationId).stream().findFirst();
    }
    ...
    public Optional<FinOpsAction> findFinOpsAction(long actionId) {
        return jdbcTemplate.query("SELECT * FROM finops_action WHERE action_id = ?", this::mapFinOpsAction,
                actionId).stream().findFirst();
    }
    ...
    public Optional<SecurityOperation> findSecurityOperation(long operationId) {
        return jdbcTemplate.query("SELECT * FROM security_operation WHERE operation_id = ?",
                this::mapSecurityOperation, operationId).stream().findFirst();
    }
    ...
    private FinOpsAction mapFinOpsAction(ResultSet rs, int rowNum) throws SQLException {
        return new FinOpsAction(rs.getLong("action_id"), rs.getString("service_name"),
                rs.getString("action_type"), rs.getBigDecimal("estimated_saving"),
