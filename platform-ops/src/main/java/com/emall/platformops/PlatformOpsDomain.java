...
@TableName("database_operation")
record DatabaseOperation(@TableId(value = "operation_id", type = IdType.INPUT) long operationId, String databaseName,
        String operationType, RiskLevel riskLevel, OpsStatus status, String detail, Instant createdAt,
        Instant updatedAt) {
    DatabaseOperation changeStatus(OpsStatus nextStatus) {
        return new DatabaseOperation(operationId, databaseName, operationType, riskLevel, nextStatus, detail, createdAt,
                Instant.now());
    }
}

@TableName("finops_action")
record FinOpsAction(@TableId(value = "action_id", type = IdType.INPUT) long actionId, String serviceName,
        String actionType, BigDecimal estimatedSaving, OpsStatus status, Instant createdAt, Instant updatedAt) {
...
record SecurityOperation(@TableId(value = "operation_id", type = IdType.INPUT) long operationId, String serviceName,
        String signalType, RiskLevel riskLevel, OpsStatus status, Instant createdAt, Instant updatedAt) {
