...
class RiskService {
    ...
    @Transactional
    RiskRule createRule(RiskScene scene, String ruleCode, String fieldName, RuleOperator operator,
                        BigDecimal threshold, RiskLevel level) {
        if (threshold.signum() < 0) {
            throw new BusinessException(ErrorCode.BAD_REQUEST, "risk threshold must not be negative");
        }
        Instant now = Instant.now();
        return repository.saveRule(new RiskRule(idGenerator.nextId(), scene, normalize(ruleCode),
                normalize(fieldName), operator, threshold, level, RuleStatus.DRAFT, now, now));
    }
    ...
    DeviceReputation upsertDevice(String deviceId, int reputationScore, boolean risky) {
        ...
        return repository.saveDevice(new DeviceReputation(normalize(deviceId), reputationScore, risky,
                Instant.now()));
    }
    ...
    RiskDecision evaluate(RiskScene scene, String subjectId, String deviceId, String ip, BigDecimal amount,
        ...
        int deviceScore = repository.findDevice(normalizedDeviceId)
                ...
                .orElse(0);
        RuleMatch match = repository.findActiveRules(scene).stream()
                .filter(rule -> matches(rule, amount, velocity, deviceScore))
                .map(rule -> new RuleMatch(rule.level(), scoreFor(rule.level()), rule.ruleCode()))
                .max(Comparator.comparingInt(RuleMatch::score))
                .orElse(defaultDecision(deviceScore));
        RiskDecision decision = new RiskDecision(match.level(), Math.max(match.score(), deviceScore),
                match.reason());
        repository.saveEvent(new RiskEvent(idGenerator.nextId(), scene, normalize(subjectId), normalizedDeviceId,
                normalize(ip), amount, velocity, decision.score(), decision.level(), decision.reason(),
                Instant.now()));
        return decision;
    }
