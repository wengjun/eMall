...
public class JdbcPaymentRepository implements PaymentRepository {
    ...
    public Optional<PaymentOrder> findById(long paymentId) {
        return jdbcTemplate.query("SELECT * FROM payment_order WHERE payment_id = ?", this::map, paymentId).stream()
                .findFirst();
    }

    @Override
    public Optional<PaymentOrder> findByRequestId(String requestId) {
        return jdbcTemplate.query("SELECT * FROM payment_order WHERE request_id = ?", this::map, requestId).stream()
                .findFirst();
    }
    ...
    private PaymentOrder map(ResultSet rs, int rowNum) throws SQLException {
        return new PaymentOrder(rs.getLong("payment_id"), rs.getString("request_id"), rs.getLong("order_id"),
                rs.getLong("user_id"), rs.getBigDecimal("amount"), rs.getString("channel"),
                rs.getString("channel_trade_no"), PaymentStatus.valueOf(rs.getString("status")),
                rs.getBoolean("order_confirmed"), rs.getTimestamp("created_at").toInstant(),
