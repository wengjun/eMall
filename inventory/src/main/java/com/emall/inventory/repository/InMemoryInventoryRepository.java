...
public class InMemoryInventoryRepository implements InventoryRepository {
    ...
    public List<InventoryBucket> findBuckets(long skuId) {
        return buckets.values().stream()
                .filter(bucket -> bucket.skuId() == skuId)
                .sorted(Comparator.comparing(InventoryBucket::bucketNo))
                .toList();
    }
    ...
    public Optional<InventoryBucket> findReservableBucket(long skuId, int quantity) {
        return findBuckets(skuId).stream()
                .filter(bucket -> bucket.available() >= quantity)
    ...
    public List<InventoryReservation> findExpiredReservations(Instant now, int limit) {
        return reservations.values().stream()
                ...
                .sorted(Comparator.comparing(InventoryReservation::expiresAt))
                .limit(limit)
                .toList();
    }
